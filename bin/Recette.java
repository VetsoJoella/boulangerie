...
public class Recette {
    ...
    public static Recette[] getByCriteriaType(Connection connection, Ingredient ingredient, Type type) throws Exception {
       
        List<Recette> recettes = new ArrayList<>();


        String sql = "select * from  v_recette_produit_details where 1=1 " ;
        if(type!=null && type.getId()!=null && !type.getId().isEmpty()) sql+= "and idType = ? ";
        else sql+= "and '1' = ? ";
        if(ingredient!=null && ingredient.getId()!=null && !ingredient.getId().isEmpty()) sql+= "and idIngredient = ? ";
        else sql+= "and '1' = ? ";

        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            if(type!=null && type.getId()!=null && !type.getId().isEmpty()) pstmt.setString(1, type.getId());
            else pstmt.setString(1, "1");
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                while (rs.next()) {
                    Produit p= new Produit(rs.getString("idProduit"), rs.getString("nomProduit"), rs.getDouble("d_prixVente")); 
                    Ingredient i = new Ingredient(rs.getString("idIngredient"));
                    i = Ingredient.getById(connection, i.getId());
