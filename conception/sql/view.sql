...
create or replace view v_recette_fabrication_produit_details as 
    select v.*, p.nom as nomProduit, p.d_prixvente from v_recette_fabrication_produit v join produit p on p.id = idProduit;




--- Produit caracteristique : 
create or replace view v_produit_caracteristique as 
    select v.*, p.nom as nom ,d_prixvente, idType, c.nom as nomCaracteristique  from produitcaracteristique v join produit p on p.id = idProduit join caracteristique c on c.id =idCaracteristique;


-- Vente caracteristique 
create or replace view v_vente_produit_caracteristique as 
    select v.*, p.nom as nomProduit ,d_prixvente, idType, idCaracteristique  from vente v join produitcaracteristique pc on v.idProduit = pc.idProduit join produit p on p.id =pc.idCaracteristique;